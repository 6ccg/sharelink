...
推荐使用 Docker Compose 部署。完整的首次部署、`.env` 配置、更新和停止命令见[部署说明](#部署说明)。
...
默认 `docker-compose.yml` 使用 GHCR 镜像 `ghcr.io/6ccg/sharelink:latest`，并把运行数据保存到项目根目录的 `./data`。部署目录至少需要这两个文件：
...
启动后访问 `http://localhost:8080/admin`。默认用户名是 `url`，密码是 `.env` 里的 `INITIAL_ADMIN_PASSWORD`。该密码只在首次初始化空数据库时写入，后续请在后台设置页修改密码。

查看运行状态和日志：

```bash
docker compose ps
docker compose logs -f sharelink
```
...
`docker compose down` 不会删除 `./data` 里的 SQLite 数据库。不要随意删除 `./data`，否则会丢失短链、访问日志和后台设置。
