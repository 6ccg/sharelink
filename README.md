...
推荐使用 Docker Compose 部署。仓库里的 `docker-compose.yml` 会从 GHCR 拉取镜像，但启动前必须先准备 `.env` 配置文件。
...
2. 修改 `.env` 中的关键配置：
   ...
   `INITIAL_ADMIN_PASSWORD` 是首次初始化管理员密码，默认登录用户名是 `url`。`JWT_SECRET` 用于签名登录会话，生产环境请使用随机长字符串。
...
默认 `docker-compose.yml` 使用 GHCR 镜像，并把运行数据保存到项目根目录的 `./data`。部署目录至少需要这两个文件：
